...
def main():
    ...
    unbroken_mode = config['model'].get('unbroken_dataset', False)
    
    if unbroken_mode:
        print("✓ Unbroken dataset mode enabled - no chunking will be applied")
        chunker = None
    else:
        try:
    ...
    # Open output file
    output_path = config['output']['unbroken_file'] if unbroken_mode else config['output']['file']
    print(f"\n[4/5] Creating output file: {output_path}")
    if unbroken_mode:
        print("  Mode: Unbroken (complete entries, no chunking)")
    else:
        print(f"  Mode: Chunked (max {config['model']['max_chunk_size']} tokens, {config['model']['chunk_overlap']} overlap)")
    ...
    with open(output_path, 'w', encoding='utf-8') as outfile:
        ...
        for file_data in repo_extractor.extract_files(show_progress=config['performance']['show_progress']):
            ...
            if unbroken_mode or chunker is None:
                # Write complete entry without chunking
                write_jsonl(file_data, outfile)
                total_chunks += 1
            else:
        ...
        for commit_data in git_extractor.extract_commits(
            ...
            if unbroken_mode or chunker is None:
                # Write complete entry without chunking
                write_jsonl(commit_data, outfile)
                total_chunks += 1
            else:
                chunks = chunker.chunk_text(
    ...
    if unbroken_mode:
        print(f"Total items: {total_chunks} (unbroken - no chunking applied)")
    else:
    ...
    if unbroken_mode:
        print("\nUnbroken mode: Each entry is stored complete without chunking.")
        print("You can chunk dynamically during training to any sequence length.")
    else:
        print(f"\nChunked mode: Max {config['model']['max_chunk_size']} tokens per chunk")
        print(f"with {config['model']['chunk_overlap']} token overlap.")
